...
class SingleFitting:
    ...
    def modelSingleFitting(self, saveAs, extra=False, pearce=False, init=False, asym=False, transfer=None):
        """
        ...
        for run in range(0, max(self.subjectData.runNumber)):
            ...
            for j in range(0, self.gridCount):
                ...
                actionProb[:] = np.nan
                V_option = np.empty((max(runData.trialNumber) + 1, 3, 3))
                V_option[:] = np.nan
                if init:
                    V_option[0, :] = V_optionInit_Grid[j]
                else:
                    V_option[0, :] = 0.5
                ...
                if init:
                    NLL_array[run, j, 6] = V_optionInit_Grid[j][0][0]
                if transfer is not None:
                    NLL_array[run, j, 7] = K1Check
                if transfer == "two":
                    NLL_array[run, j, 8] = K2Check
                elif transfer == "four":
                    NLL_array[run, j, 8] = K2Check
                    NLL_array[run, j, 9] = K3Check
                    NLL_array[run, j, 10] = K4Check
        ...
        if self.plotting:
            self.Plot.plots_modelSingleFitting(
                NLL_array=NLL_array,
                ...
                betas=fitted_betas,
                pearce=pearce,
                V_optionInits=fitted_V_optionInits,
                K1=fitted_K1,
                K2=fitted_K2,
                K3=fitted_K3,
                K4=fitted_K4,
