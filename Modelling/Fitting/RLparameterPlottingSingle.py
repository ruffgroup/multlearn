...
class SinglePlotting:
    ...
    def plots_modelSingleFitting(
        ...
        for run in range(0, max(subjectData.runNumber)):
            ...
            if not np.isnan(V_optionInits).all():
                V_option = V_optionInits[run]
                varList.append("\nV init: {0}".format(np.round(V_option[0][0], 2)))
            else:
                V_option = None
            ...
            if simA_lines.size:
                ...
                save_name = "{0}_{1}_{2}_SinglePlots.pdf".format(self.ID, run, saveAs)
