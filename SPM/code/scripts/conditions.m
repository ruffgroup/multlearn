...
if contains(model_version, "spe") && ~contains(model_version, "rpe") 
    spe = load(['/multlearn-sns/Modelling/Fitting/bestFittingVals/sub-' subject '/' model_version '.mat']).spe;

elseif contains(model_version, "rpe") && ~contains(model_version, "spe")
    if contains(model_version, "Best")
        rpe = load(['/multlearn-sns/Modelling/Fitting/bestFittingVals/sub-' subject '/rpe*.mat']).rpe.rpe;
    else
        rpe = load(['/multlearn-sns/Modelling/Fitting/' model_version '/sub-' subject '/' model_version '.mat']).rpe;
    end

elseif contains(model_version, "_")
    model = split(model_version, '_');
    spe = load(['/multlearn-sns/Modelling/Fitting/bestFittingVals/sub-' subject '/' char(model(1,1)) '.mat']).spe;
    if contains(model_version, "Best")
        rpe = load(['/multlearn-sns/Modelling/Fitting/bestFittingVals/sub-' subject '/rpe*.mat']).rpe.rpe;
    else
        rpe = load(['/multlearn-sns/Modelling/Fitting/' char(model(2,1)) '/sub-' subject '/' char(model(2,1)) '.mat']).rpe;
    end
end
