function func_level2_SnPM_ROI(path,subs, model_version, contrast_num, splitting, ROI_folder, ROI)
...
elseif ischar(splitting) && splitting == string(model_version) || ~ischar(splitting)
    ...
    data.destination = fullfile(path.SPM_folder,'/results', model_version, 'ROI', ROI_folder, strcat('Second_level_SnPM_con', num2str(contrast_num )), filesep );
end
...
matlabbatch{1}.spm.tools.snpm.des.OneSampT.masking.em = {strcat(ROI.folder,ROI.name, ',1')};
