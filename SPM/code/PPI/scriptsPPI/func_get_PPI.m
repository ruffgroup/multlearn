...
VOIfolder = string(fullfile(SPM_folder, '/results', model_version, 'ROI_analysis', ROI_folder, ROI_name, splitting,sub));
    ...
    for nrun=1:6
        ...
        if contains(splitting,'spe')
            matlabbatch{1}.spm.stats.ppi.type.ppi.u = [1 1 0
                ...
                2 1 0
                2 2 0];
            matlabbatch{1}.spm.stats.ppi.name = strcat('spe','_',char(ROI_folder),'_',char(ROI_name),'_',num2str(nrun));
        elseif contains(splitting,'rpe')
            matlabbatch{1}.spm.stats.ppi.type.ppi.u = [1 1 0
                1 2 0
                2 1 0
                2 2 1];
            matlabbatch{1}.spm.stats.ppi.name = strcat('rpe','_',char(ROI_folder),'_',char(ROI_name),'_',num2str(nrun));
        end
